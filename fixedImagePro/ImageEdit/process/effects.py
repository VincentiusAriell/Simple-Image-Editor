...
def apply_effects_cv2(cv2_img,
                      ...
                      edge=0.0,      # Edge Detection toggle
                      edge_mode='canny'):
    ...
    if edge > 0.5: # Toggle
        ...
        mode = (edge_mode or 'canny').lower()
        if mode == 'canny':
            # Canny Edge Detection
            edges = cv2.Canny(gray, 100, 200)
            arr = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR).astype(np.float32)
        elif mode == 'sobel':
            # Sobel: compute gradient magnitude and normalize
            ...
            # Optional threshold to make edges crisper
            _, th = cv2.threshold(mag_u8, 50, 255, cv2.THRESH_BINARY)
            arr = cv2.cvtColor(th, cv2.COLOR_GRAY2BGR).astype(np.float32)
        else:
            # Fallback ke canny kalau mode tidak dikenali
            edges = cv2.Canny(gray, 100, 200)
            arr = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR).astype(np.float32)
